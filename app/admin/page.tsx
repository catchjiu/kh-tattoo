...
export default async function AdminDashboardPage() {
  ...
  const { data: recentBookings } = await supabase
    .from("bookings")
    .select("id, name, email, style, created_at")
    .order("created_at", { ascending: false })
    .limit(5);

  const cards = [
    { href: "/admin/bookings", label: "Bookings", icon: CalendarCheck, count: bookingCount ?? 0, highlight: (bookingCount ?? 0) > 0 },
              ...
              <tbody>
                {recentBookings.map((b) => (
                  <tr key={b.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--card-hover)]">
                    ...
                    <td className="px-4 py-3 text-sm text-[var(--muted)]">{b.email}</td>
                    <td className="px-4 py-3 text-sm">{b.style || "—"}</td>
